...
# (But the lists do not affect the Big Oh runtime)
# The key used is the match amino acid sequence and
# the value is a list containing the alignment title (protein name)
    ...
    for alignment in blast_record.alignments:
        for hsp in alignment.hsps:
            if hsp.expect < E_VALUE_THRESH:
                cd_dict[hsp.match + blast_record.query + alignment.title] = [
                    blast_record.query, hsp.query, alignment.title, hsp.sbjct]
...
for blast_record in dc_parse:
    for alignment in blast_record.alignments:
        for hsp in alignment.hsps:
            if hsp.expect < E_VALUE_THRESH:
                dc_dict[hsp.match + alignment.title + blast_record.query] = [
                    blast_record.query, hsp.query, alignment.title, hsp.sbjct]
...
# If not then, the definition of a reciprocal match is
# both a HSP from species A to B and species B to A

if len(sys.argv) == 4:
    # iterating through all the HSPs from
    # the celegens_drosophila generated dict
    for key, value in cd_dict.items():
        # if there is a matched seq that is the same or
        # reciprocal homologs then add it to the output file
...
# FALSE opt agument means the definition of a reciprocal match
# is no longer both a HSP from species A to B and
# species B to A, but is defined as
# a HSP from species A to B or species B to A.
else:
    # if the fourth argument is FALSE then
    if sys.argv[4] == 'FALSE':
        for key, value in cd_dict.items():
            # the key will be concatenation of the query protein name
            # and hit/subject protein name
    ...
    # if the fourth cmd line argument is not FALSE
    # then return an error and quit
    else:
